...
function countValues(values: string[], limit = 6) {
...
<section className="mb-8 grid gap-6 xl:grid-cols-3">
  <Panel
    ...
    description="Top observables extracted from reports and finding evidence."
  >
    <div className="grid gap-4">
      <IntelList
        title="Top IP addresses"
        items={data.topIps}
        emptyMessage="No IP indicators detected yet."
      />

      <IntelList
        title="Top domains"
        items={data.topDomains}
        emptyMessage="No domain indicators detected yet."
      />
    </div>
  </Panel>
  ...
  <Panel
    ...
    description="CVEs and MITRE techniques found across the current workspace."
  >
    <div className="grid gap-4">
      <IntelList
        title="Top CVEs"
        items={data.topCves}
        emptyMessage="No CVE references detected yet."
      />

      <IntelList
        title="Top MITRE techniques"
        items={data.topMitre}
        emptyMessage="No MITRE technique IDs detected yet."
      />
    </div>
  </Panel>
  ...
  <Panel
    ...
    description="Hash indicators and export readiness signals."
  >
    <div className="grid gap-4">
      <IntelList
        title="Top hashes"
        items={data.topHashes}
        emptyMessage="No file hash indicators detected yet."
      />

      <div className="rounded-[20px] border border-[#dcefe2] bg-[#fbfffd] p-4">
        <p className="text-sm font-semibold text-[#0d2217]">Export readiness</p>
        <p className="mt-2 text-3xl font-semibold text-[#14532d]">
          {data.readyForExport.length}
        </p>
        <p className="mt-1 text-sm leading-6 text-[#5a7668]">
          reports have findings that do not require analyst review.
        </p>
        <Link
          href="/export"
          className="mt-4 inline-flex rounded-2xl border border-[#cbe8d6] bg-white px-4 py-2 text-sm font-semibold text-[#14532d] hover:bg-[#edfdf3]"
        >
          Open export center
        </Link>
      </div>
...
function IntelList({
  ...
  title: string
  items: Array<{ id: string; value: string; count: number }>
  emptyMessage: string
}) {
  return (
    <div className="rounded-[20px] border border-[#dcefe2] bg-[#fbfffd] p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <p className="text-sm font-semibold text-[#0d2217]">{title}</p>
        <Badge tone={items.length > 0 ? 'success' : 'neutral'}>{items.length}</Badge>
      </div>

      {items.length === 0 ? (
        <p className="text-sm leading-6 text-[#5a7668]">{emptyMessage}</p>
      ) : (
        <div className="space-y-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between gap-3 rounded-2xl border border-[#e4f2e9] bg-white px-3 py-2"
            >
              <span className="min-w-0 truncate font-mono text-xs text-[#173128]">
                {item.value}
              </span>
              <span className="rounded-full bg-[#edfdf3] px-2 py-1 text-xs font-semibold text-[#15803d]">
                {item.count}
              </span>
            </div>
          ))}
