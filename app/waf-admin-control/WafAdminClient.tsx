...
export default function WafCenterPage() {
  ...
  async function logoutAdmin() {
    await fetch('/api/admin/waf/session', { method: 'DELETE', cache: 'no-store' }).catch(() => undefined)
  ...
  async function runDemo(testCase: DemoCase) {
    if (!summary) {
      setError('Admin session is not ready. Refresh the page and sign in again.')
      return
    }
      ...
      <section className="mx-auto max-w-7xl space-y-8">
        <div className="overflow-hidden rounded-[2rem] border border-emerald-100 bg-white shadow-sm">
          <div className="grid gap-0 lg:grid-cols-[1fr_410px]">
            ...
            <div className="border-t border-emerald-100 bg-[#f7fbf8] p-5 lg:border-l lg:border-t-0">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5a7668]">Owner Session</p>
              <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900">
                You are signed in to the owner-only WAF console. The admin token is never shown in the browser and is not stored in localStorage.
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                >
                  {loading ? 'Loading...' : 'Refresh WAF Data'}
                </button>
                <button
                  onClick={logoutAdmin}
              ...
              <p className="mt-3 text-xs leading-5 text-[#6b8277]">Keep this page open during controlled WAF demos. Normal users cannot reach this console from the application navigation.</p>
            </div>          </div>
        </div>
        ...
        {summary ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-emerald-200 bg-white p-10 text-center text-[#5a7668]">
            Your owner session is not loaded. Refresh this page or sign in again from the owner gate.
          </div>
        )}
      </section>
